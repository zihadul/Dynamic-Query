...
public partial class Pages_Setup_AprGradeSetupList : System.Web.UI.Page
{
    ...
    {
        gv.DataSource = new bllAprGrade().GetList();
        gv.DataBind();
    }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllAprGrade().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllAprGrade().Delete(id);
            new bllAudit().Insert("Setup", "Delete AprGrade", "AprGrade: " + dt.Rows[0]["Description"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
