...
public partial class Pages_Setup_CadreSetup : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["CadreCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["CadreCriteria"] = Criteria;
        }

        dt = new bllCadre().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxCadre.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "Cadre like '%" + tbxCadre.Text.Trim() + "%'";
            else Criteria += " And Cadre like '%" + tbxCadre.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllCadre().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllCadre().Delete(id);
            new bllAudit().Insert("Setup", "Delete Cadre", "Cadre: " + dt.Rows[0]["Cadre"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
