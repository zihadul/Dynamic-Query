...
public partial class Pages_Setup_MedicalCategoryList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["MedicalCategoryCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["MedicalCategoryCriteria"] = Criteria;
        }

        dt = new bllMedicalCategory().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxMedicalCategory.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "MedicalCategoryName like '%" + tbxMedicalCategory.Text.Trim() + "%'";
            else Criteria += " And MedicalCategoryName like '%" + tbxMedicalCategory.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllMedicalCategory().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllMedicalCategory().Delete(id);
            new bllAudit().Insert("Setup", "Delete Medical Category", "Medical Category: " + dt.Rows[0]["MedicalCategoryName"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
    ...
    protected void managePaging()
    {
        if (TotalRec > 0)
        {
            ...
            strSummary.Append(numberOfRecords.ToString());
            strSummary.Append("</b> Medical Category</br>");
