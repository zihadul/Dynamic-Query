...
public partial class Pages_Setup_SpecialQualificationList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["SpclQltyCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["SpclQltyCriteria"] = Criteria;
        }

        dt = new bllSpecialQuality().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxSpclQulty.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "SpecialQualityName like '%" + tbxSpclQulty.Text.Trim() + "%'";
            else Criteria += " And SpecialQualityName like '%" + tbxSpclQulty.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllSpecialQuality().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllSpecialQuality().Delete(id);
            new bllAudit().Insert("Setup", "Delete Special Quality", "Special Quality: " + dt.Rows[0]["SpecialQualityName"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
    ...
    protected void managePaging()
    {
        if (TotalRec > 0)
        {
            ...
            strSummary.Append(numberOfRecords.ToString());
            strSummary.Append("</b> Special Quality</br>");
