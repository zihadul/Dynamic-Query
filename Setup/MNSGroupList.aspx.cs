...
public partial class Pages_Setup_MNSGroupList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["MSNGroupCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["MSNGroupCriteria"] = Criteria;
        }

        dt = new bllMNSGroup().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxMSNGroupName.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "MNSGroupName like '%" + tbxMSNGroupName.Text.Trim() + "%'";
            else Criteria += " And MNSGroupName like '%" + tbxMSNGroupName.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllMNSGroup().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllMNSGroup().Delete(id);
            new bllAudit().Insert("Setup", "Delete MNS Group", "MNS Group: " + dt.Rows[0]["MNSGroupName"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
    ...
    protected void managePaging()
    {
        if (TotalRec > 0)
        {
            ...
            strSummary.Append(numberOfRecords.ToString());
            strSummary.Append("</b> MSN Group</br>");
