...
public partial class Pages_Setup_MSNList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["MSNCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["MSNCriteria"] = Criteria;
        }

        dt = new bllMSN().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxMSN.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "MsnName like '%" + tbxMSN.Text.Trim() + "%'";
            else Criteria += " And MsnName like '%" + tbxMSN.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllMSN().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllMSN().Delete(id);
            new bllAudit().Insert("Setup", "Delete MNS", "MNS: " + dt.Rows[0]["MsnName"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
