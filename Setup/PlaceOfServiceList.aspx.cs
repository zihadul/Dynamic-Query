...
public partial class Pages_Setup_PlaceOfServiceSetup : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["PlaceCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["PlaceCriteria"] = Criteria;
        }

        dt = new bllPlaceOfService().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxPlaceOfService.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "PlaceOfService like '%" + tbxPlaceOfService.Text.Trim() + "%'";
            else Criteria += " And PlaceOfService like '%" + tbxPlaceOfService.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllPlaceOfService().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllPlaceOfService().Delete(id);
            new bllAudit().Insert("Setup", "Delete Place Of Service", "Panishment Place Of Service: " + dt.Rows[0]["PlaceOfService"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
