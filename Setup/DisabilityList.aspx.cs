...
public partial class Pages_Setup_DisabilityList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["ArmsCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["ArmsCriteria"] = Criteria;
        }

        dt = new bllDisablity().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxDiablity.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "Diablity like '%" + tbxDiablity.Text.Trim() + "%'";
            else Criteria += " And Diablity like '%" + tbxDiablity.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllDisablity().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllDisablity().Delete(id);
            new bllAudit().Insert("Setup", "Delete Disablity", "Disablity: " + dt.Rows[0]["Diablity"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
