...
public partial class Pages_Setup_UnitList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["UnitCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["UnitCriteria"] = Criteria;
        }

        dt = new bllUnit().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxUnitName.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "UnitName like '%" + tbxUnitName.Text.Trim() + "%'";
            else Criteria += " And UnitName like '%" + tbxUnitName.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllUnit().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllUnit().Delete(id);
            new bllAudit().Insert("Setup", "Delete Unit", "Unit: " + dt.Rows[0]["UnitName"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
