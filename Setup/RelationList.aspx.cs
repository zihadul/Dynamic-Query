...
public partial class Pages_Setup_RelationSetup : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["RelationCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["RelationCriteria"] = Criteria;
        }

        dt = new bllRelation().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxRelation.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "Relation like '%" + tbxRelation.Text.Trim() + "%'";
            else Criteria += " And Relation like '%" + tbxRelation.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllRelation().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllRelation().Delete(id);
            new bllAudit().Insert("Setup", "Delete Relation", "Relation: " + dt.Rows[0]["Relation"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
    ...
    protected void managePaging()
    {
        if (TotalRec > 0)
        {
            ...
            strSummary.Append(numberOfRecords.ToString());
            strSummary.Append("</b> Relation</br>");
