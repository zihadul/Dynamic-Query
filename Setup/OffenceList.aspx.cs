...
public partial class Pages_Setup_OffenceList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["OffenceCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["OffenceCriteria"] = Criteria;
        }

        dt = new bllOffence().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxOffence.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "OffenceName like '%" + tbxOffence.Text.Trim() + "%'";
            else Criteria += " And OffenceName like '%" + tbxOffence.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllOffence().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllOffence().Delete(id);
            new bllAudit().Insert("Setup", "Delete Offence", "Offence: " + dt.Rows[0]["OffenceName"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
    ...
    protected void managePaging()
    {
        if (TotalRec > 0)
        {
            ...
            strSummary.Append(numberOfRecords.ToString());
            strSummary.Append("</b> Offence</br>");
