...
public partial class Pages_Setup_GradeSetupList : System.Web.UI.Page
{
    ...
    {
       gv.DataSource= new bllGrade().GetList();
       gv.DataBind();
    }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllGrade().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllGrade().Delete(id);
            new bllAudit().Insert("Setup", "Delete Grade", "Latter Grade: " + dt.Rows[0]["LatterGrade"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
