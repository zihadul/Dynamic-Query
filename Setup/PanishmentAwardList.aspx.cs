...
public partial class Pages_Setup_PanishmentAwardList : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["PunishmentCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["PunishmentCriteria"] = Criteria;
        }

        dt = new bllPanishmentAwarded().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxPunishmentAward.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "Name like '%" + tbxPunishmentAward.Text.Trim() + "%'";
            else Criteria += " And Name like '%" + tbxPunishmentAward.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllPanishmentAwarded().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllPanishmentAwarded().Delete(id);
            new bllAudit().Insert("Setup", "Delete Panishment Award", "Panishment Award: " + dt.Rows[0]["Name"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
