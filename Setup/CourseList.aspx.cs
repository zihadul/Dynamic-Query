...
public partial class Pages_Setup_CourseSetup : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            Criteria = Session["CourseCriteria"].ToString();

        }
        else
        {
            Criteria = GetCriteria();
            Session["CourseCriteria"] = Criteria;
        }

        dt = new bllCourse().GetBySearchParameter(CurrentPage * PageSize, PageSize, Criteria, PrintMode);
        gv.DataSource = dt;
        gv.DataBind();
        if (dt.Rows.Count > 0)
        {
    ...
    private string GetCriteria()
    {
        ...
        if (tbxCourse.Text.Trim() != "")
        {
            if (Criteria == "") Criteria = "Course like '%" + tbxCourse.Text.Trim() + "%'";
            else Criteria += " And Course like '%" + tbxCourse.Text.Trim() + "%'";
        }
    ...
    protected void btnDelete_Command(object sender, CommandEventArgs e)
    {
        try
        {
            DataTable dt = new bllCourse().GetById(Convert.ToInt32(e.CommandArgument.ToString()));
            int id = Convert.ToInt32(e.CommandArgument.ToString());
            new bllCourse().Delete(id);
            new bllAudit().Insert("Setup", "Delete Course", "Course: " + dt.Rows[0]["Course"].ToString(), User.Identity.Name); 
            BindData();
            managePaging();
        }
        catch (Exception ex)
